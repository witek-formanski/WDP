...
/*** ponizej okropnie slaba proba zrobienia +1 z uwzglednieniem roznych
 * przypadkow brzegowych ***/
...
bool inc(int cyfra[], int wynik[], int size) {
    ...
    // w przec. przyp. (koniec 01) -> zmien na 10 i dodaj 1 do prefiksu
    // kodlugosci 2
    if (cyfra[0] == 1 && cyfra[1] != 1) {
        // tu sie poddalem
        return false;
...
int main(void) {
    ...
    for (int i = 0; (i < size) && (wynik[i] != -1); ++i) printf("%d", wynik[i]);
    printf("\n");
