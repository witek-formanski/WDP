...
ciag* diffSeq(ciag* s) {
    ...
    for (int i = n - 2; i >= 0; --i) diffEl[i] = (s->el)[i + 1] - (s->el)[i];
    diff->el = diffEl;
    return diff;
}
...
int main(void) {
    ...
    ciag* t = diffSeq(s);
    for (int i = 0; i < n - 1; ++i) printf("%f ", (t->el)[i]);
    printf("\n");

    ciagCiagow* ss = diffSeqs(s);
    while (ss) {
        for (int i = 0; i < (ss->c)->dl; i++) printf("%f ", (ss->c)->el[i]);
        printf("\n");
        ss = ss->nast;
    }
    ...
    free(t);
    free(s);
    free(ss);  // to powinno byc madrzejsze (jakas procedura iteracyjnie
               // zwalniaca ss od konca)
